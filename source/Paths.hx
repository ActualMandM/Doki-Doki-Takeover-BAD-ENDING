...
class Paths
{
	...
	{
		#if MODS_ALLOWED
		if (!ignoreCheck && ClientPrefs.imagesPersist)
	...
	inline static public function image(key:String, ?library:String, isLocale:Bool = false):Dynamic
	{
		#if MODS_ALLOWED
		...
		#end
		if (isLocale)
			return getPath('locale/' + ClientPrefs.language + '/images/$key.png', IMAGE, library);
		else
	...
	inline static public function getSparrowAtlas(key:String, ?library:String, isLocale:Bool = false)
	{
		...
		if (isLocale)
			return FlxAtlasFrames.fromSparrow((imageLoaded != null ? imageLoaded : image(key, library, isLocale)),
				(xmlExists ? File.getContent(modsXml(key)) : file('locale/' + FlxG.save.data.language + '/images/$key.xml', library)));
		else
